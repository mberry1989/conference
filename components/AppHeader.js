...
import { filterPageMapBySlugAndCollection } from "@utils/helpers";
...
const AppHeader = ({ collection, hasExtendedNav = false, backBtn = false }) => {
  ...
  useEffect(() => {
    ...
    async function fetchNav() {
      const response = await fetch("/api/navigationFetcher");
  ...
  const navigtionPages = navigation.filter(
    (navItem) =>
      navItem.value["showInNavigation"] === "yes" &&
      navItem.value["collection"] === collection
  );

  const menuItems = navigtionPages.map((item) => ({
            ...
            <div className="text-gray-600 dark:text-gray-300 lg:pr-4 lg:w-auto w-full lg:pt-0">
              <ul className="tracking-wide font-medium lg:text-sm flex-col flex lg:flex-row gap-6 lg:gap-0">
                {menuItems.map((item, index) => {
                  const url = filterPageMapBySlugAndCollection(
                    navigation,
                    collection,
                    item.slug
                  ).value.url;
